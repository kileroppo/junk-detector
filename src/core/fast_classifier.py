...
def train_from_history(db_path: str = "junk_detector.db", output_path: str | None = None) -> dict:
    """Train the FastClassifier from historical scoring data.
    
    Reads scored articles from the database, extracts features from stored text,
    and trains a RandomForestClassifier.
    
    Args:
        db_path: Path to the SQLite database with scoring history.
        output_path: Where to save the model. Default: models/fast_classifier.pkl
    
    Returns:
        Dict with training metrics (accuracy, samples_used, etc.)
    """
    try:
        import numpy as np
        ...
        raise ImportError("sklearn is required for training. Install with: pip install scikit-learn")
    
    from src.storage.db import query
    
    # Load all scored records
    records = query(filters=None, limit=10000, db_path=db_path)
    
    if len(records) < 20:
        raise ValueError(f"Not enough training data. Need at least 20 records, got {len(records)}")
    ...
    for record in records:
        # We need text content — reconstruct from what we have
        # Use title + summary as proxy (we don't store full text)
        text = (record.get("title", "") or "") + " " + (record.get("summary", "") or "")
        ...
        score = record.get("overall_score", 50)
        if score < 30:
            y_data.append("junk")
        elif score < 50:
            y_data.append("low")
        elif score < 70:
            y_data.append("medium")
        else:
            y_data.append("good")
    
    if len(X_data) < 20:
        raise ValueError(f"Not enough usable training samples. Got {len(X_data)}")
    ...
    # Save model
    save_path = Path(output_path) if output_path else _MODEL_PATH
    ...
    logger.info(f"FastClassifier trained: accuracy={accuracy:.2%}, samples={len(X_data)}, saved to {save_path}")
    ...
    return {
        ...
        "class_distribution": {c: int((y == c).sum()) for c in set(y)},
    }
