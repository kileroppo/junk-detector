...
async def hydrate_similar_articles(
    ctx: "PipelineContext",  # noqa: F821
    db_path: str = "junk_detector.db",
) -> dict[str, Any]:
    """Find similar previously-scored articles using embeddings.

    Returns:
        Dict with keys:
        - similar_articles: list of similar article dicts with similarity_score
        - max_similarity: highest similarity score found (0.0 if none)
    """
    from src.core.embeddings import find_similar
    ...
    try:
        similar = await find_similar(
            text=text,
            threshold=0.80,  # slightly lower threshold to catch more potential copies
            top_k=5,
            db_path=db_path,
        ...
        if similar:
            result["max_similarity"] = similar[0]["similarity_score"]
    except Exception as e:
