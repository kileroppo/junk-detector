"""Tests for the side_effects module: base, notification, stats_collector, webhook_callback."""
...
# ===== WebhookCallbackSideEffect Tests =====


class TestWebhookCallbackSideEffect:
    """Tests for the WebhookCallbackSideEffect."""

    async def test_name(self):
        effect = WebhookCallbackSideEffect(callback_url="https://callback.example.com")
        assert effect.name == "webhook_callback"
    ...
    async def test_execute_posts_correct_payload(self):
        ctx = _make_context(overall_score=72, labels=["quality"])
        effect = WebhookCallbackSideEffect(callback_url="https://callback.example.com/hook")
        ...
        with patch("httpx.AsyncClient") as mock_client_cls:
            ...
            mock_client.post.assert_called_once()
            call_args = mock_client.post.call_args
            assert call_args[0][0] == "https://callback.example.com/hook"
            payload = call_args[1]["json"]
            assert payload["overall_score"] == 72
            assert payload["labels"] == ["quality"]
    ...
    async def test_execute_include_content(self):
        ctx = _make_context()
        effect = WebhookCallbackSideEffect(
        ...
        with patch("httpx.AsyncClient") as mock_client_cls:
            ...
            assert payload["text_preview"] == "This is test content for scoring."

    async def test_execute_include_content_false(self):
        ctx = _make_context()
        effect = WebhookCallbackSideEffect(
            callback_url="https://callback.example.com/hook",
    ...
    async def test_execute_handles_httpx_failure(self, caplog):
        ctx = _make_context()
        effect = WebhookCallbackSideEffect(callback_url="https://callback.example.com/hook")
        ...
        with patch("httpx.AsyncClient") as mock_client_cls:
            ...
            with caplog.at_level(logging.WARNING, logger="side_effects.webhook_callback"):
                await effect.execute(ctx)

            assert "failed" in caplog.text
            assert "callback.example.com" in caplog.text
    ...
    async def test_execute_dimensions_payload(self):
        """Dimensions are serialized correctly in the payload."""
        ctx = _make_context()
        effect = WebhookCallbackSideEffect(callback_url="https://callback.example.com/hook")
        ...
        with patch("httpx.AsyncClient") as mock_client_cls:
            ...
            assert dims["originality"] == 50
            assert dims["info_density"] == 40
            assert dims["scam_prob"] == 10
